...
describe("EventRouter", () => {
  describe("GET:/events", () => {
    ...
    it("should return array sorted ASCENDING by seedId field from /api/events?sort=seedId:asc GET", async () => {
      ...
      const { status, body } = await request(app).get(
        "/api/events?sortField=seedId:asc"
      );
    ...
    it("should return array sorted DESCENDING by createdDateTime field from /api/events?sort=createdDateTime+des GET", async () => {
      getAllEventsSpy.mockResolvedValue(events);
      getAllSeedsSpy.mockResolvedValue(seeds);
      ...
      const { status, body } = await request(app).get(
        "/api/events?sortField=createdDateTime+des"
      );
